    ...
    const torch::Device& default_device)
{
    ...
    // Override default_device for all sparse (expert) nodes to use the same device
    // that Python's torch.cuda.set_device() selected. This prevents expert weights
    // from being loaded to device 0 when Python expects them on a different device.
...
std::int64_t ArcherPrefetchHandle::GetDeviceMemoryCapacity(const torch::Device& device) const
{
