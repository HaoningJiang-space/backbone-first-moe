...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/results/qwen_real_machine_backbone_abd}"
...
if [ $((NUM_MEASURED_PAIRS % 3)) -ne 0 ]; then
  echo "NUM_MEASURED_PAIRS must be a multiple of 3 for balanced A/B/D order, got ${NUM_MEASURED_PAIRS}" >&2
  exit 2
fi
...
run_eval() {
  ...
  if [ "${mode}" = "B" ]; then
    ...
    extra_flags+=(--disable-backbone-lane-split --backbone-grouped-resident-mode)
  fi
...
for pair_idx in $(seq 1 "${NUM_MEASURED_PAIRS}"); do
  ...
  python - "${pair_dir}/qwen_A_heldout.json" "${pair_dir}/qwen_B_heldout.json" "${pair_dir}/qwen_D_heldout.json" "${pair_idx}" <<'PY'
import json, sys
a_path, b_path, d_path, pair_idx = sys.argv[1:5]
with open(a_path) as fa, open(b_path) as fb, open(d_path) as fd:
    a = json.load(fa)
    b = json.load(fb)
    d = json.load(fd)
def gain(x, y):
...
print(
    ...
    f"D={d['generated_tokens_per_sec']:.6f} "
    f"A->B={gain(a, b):.2f}% "
    f"B->D={gain(b, d):.2f}% "
    f"A->D={gain(a, d):.2f}%"
)
...
for pair_idx in range(1, num_pairs + 1):
    ...
    with open(pair_dir / "qwen_A_heldout.json") as fa, \
         ...
         open(pair_dir / "qwen_D_heldout.json") as fd:
        a = json.load(fa)
        b = json.load(fb)
        d = json.load(fd)
    rows.append({
...
summary = {
    ...
    "A_to_D": summarize("A_tps", "D_tps"),
}

summary_path = out_root / "summary_abd.json"
