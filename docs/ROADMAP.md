...
### What does not belong in the main claim

- per-sequence speculative prefetch
- batch-aware speculative prefetch
- throughput-sweep `k` search as the main method

Those are either legacy ablations or oracle/negative evidence. They should not define the paper claim.
...
- the repo-side packed runtime support is on `multi-model-runtime`
...
> Once this backbone is pinned, the remaining tail should be served by exact demand fallback, not speculative prefetch on the critical path.
> The resident size should be selected by burst-aware frontier feasibility, not by manual ratio tuning.
...
- a throughput sweep over resident ratios
...
### Non-incremental formulation (current method)
...
- the main path is resident backbone + demand fallback
- the tail is served by exact demand, not speculative prefetch
- the only reason to stop pinning more experts is tail feasibility

So the method should be:
- utility-ranked prefix
- constrained by burst-aware tail slack

This directly matches the runtime bottleneck.
...
- **System design** (Section 4): resident backbone + demand fallback
