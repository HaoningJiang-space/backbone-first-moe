...
class ExpertTracer:
    ...
    def _make_iter(self, stage: str, embed: torch.Tensor | None = None):
        if embed is None:
            embed = torch.zeros((self.embed_dim,), dtype=self.dtype, device=self.device)
        else:
            embed = torch.as_tensor(embed, dtype=self.dtype, device=self.device).detach()
        return {
            "stage": stage,
            "embed": embed,
    ...
    def _ensure_iters(self, trace_entry, required_len: int, stage: str):
        while len(trace_entry.iters) < required_len:
            trace_entry.iters.append(self._make_iter(stage))
    ...
    def update_embed(self, seq_id: int, embeds: torch.Tensor):
        ...
        if len(trace_entry.iters) == 0:
            for i in range(embeds.shape[0]):
                trace_entry.iters.append(self._make_iter("prefill", embeds[i]))
        else:
            assert trace_entry.iters[-1]["embed"] is not None, "Something is wrong with decode steps!"
            trace_entry.iters.append(self._make_iter("decode", embeds.detach().squeeze(0)))
    ...
    def update_entry(self, seq_id, expert_list: torch.Tensor, layer_idx: int,
        ...
        if num_tokens > 1:
            ...
            self._ensure_iters(trace_entry, num_tokens, "prefill")
            ...
            self._ensure_iters(trace_entry, token_idx + 1, "decode")
            it = trace_entry.iters[token_idx]
            states = it["states"]
            nodes = it["nodes"]
            probs = it["probs"]
